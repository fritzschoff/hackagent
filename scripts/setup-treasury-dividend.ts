/**
 * Provision the M1 TreasuryDividendDistribute workflow on KeeperHub.
 *
 * Usage:
 *   pnpm tsx scripts/setup-treasury-dividend.ts            # live
 *   pnpm tsx scripts/setup-treasury-dividend.ts --dry-run  # print spec only
 */

import { buildTreasuryDividendDistribute } from "../lib/keeperhub-workflows";
...
async function main(): Promise<void> {
  ...
  const spec = buildTreasuryDividendDistribute({ appUrl, webhookSecret });

  if (isDryRun) {
    console.log("=== DRY RUN — TreasuryDividendDistribute spec ===\n");
...
=== Next steps ===
  ...
  vercel env add KEEPERHUB_WORKFLOW_ID_DIVIDEND_DISTRIBUTE production
  # paste: ${id}

Schedule:       weekly, Sundays 00:00 UTC
Webhook target: ${appUrl}/api/keeperhub/distribute-dividend  (bearer auth)
`);
}
